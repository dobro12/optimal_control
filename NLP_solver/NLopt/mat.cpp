...
class mat{
    ...
    mat(const double* ref, int r, int c){
        row = r;
        col = c;
        data = new double[row*col];
        std::memcpy(data, ref, sizeof(double)*row*col);
    }
    mat(const mat& A){
        ...
        col = A.col;
        data = new double[row*col];
        std::memcpy(data, A.data, sizeof(double)*row*col);
    }
    ~mat(){
    ...
    const mat inverse_matmul(const mat& g, int num_conjugate=10) const{
        ...
        for(int i=0;i<g.col;i++){
            ...
            std::memcpy(&transpose_x_value.data[i*col], temp_x_value.data, sizeof(double)*col);
        }
